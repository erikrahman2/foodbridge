...
class FoodProvider extends ChangeNotifier {
  ...
  Future<void> fetchFoodsFromFirestore() async {
    try {
      ...
      _foods = snapshot.docs.map((doc) {
        ...
        data['description'] = (data['description'] ?? 'No description available').toString();
        ...
        if (kDebugMode) {
          print('🍔 Food loaded: ${data['title']} | Category: ${data['category']} | Price: ${data['price']}');
        }
  ...
  void filterByCategory(String category) {
    ...
    if (category == 'All') {
      ...
      _filteredFoods = _foods.where((food) {
        final foodCategory = (food['category'] ?? '').toString().toLowerCase().trim();
    ...
    if (kDebugMode) {
      ...
      if (_filteredFoods.isEmpty && category != 'All') {
        ...
        print('💡 Available categories: ${_foods.map((f) => f['category']).toSet()}');
      }
    }
  ...
  void toggleFavorite(String id) {
    ...
    if (index != -1) {
      ...
      if (kDebugMode) {
        print('❤️ Favorite toggled for: ${_foods[index]['title']} -> ${_foods[index]['isFavorite']}');
      }

      // Juga update di filtered foods jika ada
      final filteredIndex = _filteredFoods.indexWhere((food) => food['id'] == id);
      if (filteredIndex != -1) {
        _filteredFoods[filteredIndex]['isFavorite'] = _foods[index]['isFavorite'];
      }
